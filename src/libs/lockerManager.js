...
class LockerManager {
    ...
    chrome.tabs.onRemoved.addListener((tabId, removeInfo) => {
      ...
      Object.values(this.contexts).forEach((context) => {
        for (let i = 0; i < context.sessions.length; ++i) {
          const index = context.sessions[i].tabs.findIndex(
            (target) => target === tabId
          );
          if (index !== -1) {
  ...
  pong(request, sender) {
    ...
    Object.values(this.contexts).forEach((context) => {
      const targetSession = context.sessions.find((target) =>
        host.match(target.regexp)
      );
      if (!targetSession) {
        return;
      }
  ...
  addSession(request) {
    ...
    const existSession = targetContext.sessions.find(
